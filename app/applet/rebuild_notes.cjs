...
const lines = fs.readFileSync('src/pages/Notes.tsx', 'utf8').split('\\n');
const contentOriginal = fs.readFileSync('src/pages/Notes.tsx', 'utf8');

const startIndex = lines.findIndex(l => l.includes('export const ALL_NOTES'));
const functionIndex = lines.findIndex((l,i) => i > startIndex && l.includes('export default function Notes'));

if (startIndex === -1 || functionIndex === -1) {
  console.log('Failed to find array bounds.');
  process.exit(1);
}

const headerStr = lines.slice(0, startIndex + 1).join('\\n');
const footerStr = lines.slice(functionIndex).join('\\n');

let allNotesContent = [];
const matches = [...contentOriginal.matchAll(/id:\\s*'([^']+)'[\\s\\S]*?isExternal:\\s*(false|true)/g)];

for (const match of matches) {
  const metadataBlock = match[0];
  const idValue = match[1];
  
  // Format properties to be stringified cleanly
  let contentValue = '{}';
  if (elaborateContents[idValue]) {
    contentValue = JSON.stringify(elaborateContents[idValue], null, 4);
    ...
    // If there were other notes that weren't elaborated, let's keep them with empty objects
    contentValue = '{ intro: "", chapters: [] }';
  }
  ...
  allNotesContent.push(reconstructedObj);
}

const newNotesTsx = \`\${headerStr}
\${allNotesContent.join(',\\n')}
];

\${footerStr}\`;

fs.writeFileSync('src/pages/Notes.tsx', newNotesTsx);
console.log('Successfully re-generated ALL_NOTES array!');
