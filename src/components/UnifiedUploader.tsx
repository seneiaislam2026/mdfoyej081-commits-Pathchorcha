...
export default function UnifiedUploader() {
  const [uploadType, setUploadType] = useState<"note" | "board_question">("note");
    ...
    <div className="space-y-6 mt-4">
      <Card className="border border-indigo-100 shadow-sm rounded-2xl overflow-hidden">
        <CardHeader className="bg-indigo-50/50 border-b border-indigo-50 pb-4 p-6">
          ...
          <CardDescription className="font-bengali text-indigo-600/80">নোট বা বোর্ড প্রশ্ন সব এক জায়গা থেকে পাবলিশ করুন</CardDescription>
        </CardHeader>
        <CardContent className="p-6">
          ...
          {uploadType === "note" ? <NotesCreator /> : <BoardQuestionsCreator />}
        </CardContent>
      </Card>
    </div>
  );
