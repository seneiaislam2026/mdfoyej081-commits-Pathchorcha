...
export default function PublicExamsList() {
  ...
  return (
    <div className="min-h-screen border-t bg-muted font-bengali pb-20">
      <div className="bg-card sticky top-0 z-50 px-4 py-4 md:px-8 border-b border-slate-100">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => navigate(-1)} className="p-2 hover:bg-muted text-muted-foreground rounded-full">
              <ArrowLeft className="w-5 h-5"/>
            </button>
            <h1 className="text-xl font-bold text-foreground">পাবলিক এক্সাম ও লাইভ টেস্ট</h1>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 md:px-8 py-6">
        <div className="flex flex-col sm:flex-row gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full bg-card border border-slate-200 rounded-xl pl-10 pr-4 py-2.5 outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary"
            />
          </div>
        </div>
        ...
        {loading ? (
          ...
          <div className="text-center py-20 bg-card rounded-2xl border border-slate-100">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {filtered.map(exam => (
              <div key={exam.id} className="bg-card p-5 rounded-2xl border border-slate-100 shadow-sm hover:shadow-md transition-shadow">
                <div className="flex justify-between items-start mb-3">
                  <h3 className="font-bold text-lg text-foreground leading-tight pr-2">{exam.title}</h3>
                ...
                <div className="flex flex-wrap items-center gap-3 mb-5 text-sm text-slate-500">
                  <span className="flex items-center gap-1.5 bg-muted px-2 py-1 rounded-md text-xs">
