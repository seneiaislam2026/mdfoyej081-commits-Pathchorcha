...
import { 
  UserCircle, Target, BookOpen, Clock, LogOut, ChevronRight, ClipboardList, 
  BarChart2, Star, Database, Brain, MessageCircle, Crown, Settings, X, 
  Camera, Edit2, Save, Trash2, Plus, Bell, Calendar, User, Shield, Check, Gift
} from "lucide-react";
import { useAuth } from "../lib/AuthContext";
import { useNavigate, Link } from "react-router-dom";
import { db } from "../lib/firebase";
import { 
  collection, query, getDocs, addDoc, serverTimestamp, setDoc, doc, 
  deleteDoc, onSnapshot, where, orderBy, getDoc, updateDoc, writeBatch
...
export default function Profile() {
  ...
  const handleAddRoutineSlot = async () => {
    if (!userData?.uid) return;
    const finalSubject = selectedSubject === "অন্যান্য" ? customSubject : selectedSubject;
    ...
    try {
      const routineCollection = collection(db, "users", userData.uid, "routine");
  ...
  const dayWeight: { [key: string]: number } = {
    "শনিবার": 1, "রবিবার": 2, "সোমবার": 3, "মঙ্গলবার": 4, "বুধবার": 5, "বৃহস্পতিবার": 6, "শুক্রবার": 7
  };
    ...
    <div className="max-w-2xl mx-auto space-y-6 pb-20 px-4 pt-6">
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-6 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row items-center gap-5 text-center sm:text-left">
            ...
            <div className="relative shrink-0">
              <div id="avatar_container" className="w-[120px] h-[120px] sm:w-[130px] sm:h-[130px] rounded-full border-[5px] border-white overflow-hidden shadow-md bg-[#EEF2FF] flex items-center justify-center relative group">
                {userData?.photoURL ? (
              ...
              <label id="upload_camera_overlay" className="absolute bottom-0 right-1 bg-indigo-600 hover:bg-indigo-700 text-white p-2 sm:p-2.5 border-2 border-white rounded-full cursor-pointer transition-all hover:scale-110 shadow-md flex items-center justify-center">
            ...
            <div className="space-y-1 sm:max-w-xs">
              ...
              <div className="pt-2 text-center sm:text-left flex items-start gap-1 justify-center sm:justify-start">
                <span className="text-indigo-400 font-serif text-lg font-bold leading-none">“</span>
          ...
          <div className="hidden md:block select-none">
            <svg viewBox="0 0 160 120" className="w-[125px] h-[95px] text-indigo-600 shrink-0">
              <circle cx="10" cy="50" r="1.5" fill="#C084FC" />
              <circle cx="140" cy="20" r="2" fill="#FBBF24" />
              <circle cx="150" cy="80" r="2.5" fill="#818CF8" />

              <g transform="translate(15, 65)">
                <path d="M-5 -10 Q5 -20 15 -10 Q5 0 -5 -10 Z" fill="#818CF8" opacity="0.6" />
                <path d="M5 -15 Q-5 -30 -15 -10 Q-5 0 5 -15 Z" fill="#818CF8" />
                <path d="M0 -5 Q10 -15 0 -25 Q-10 -15 0 -5 Z" fill="#6366F1" opacity="0.8" />
                <path d="M-8 0 L8 0 L6 18 L-6 18 Z" fill="#475569" />
              </g>

              <g transform="translate(60, 45)">
                <rect x="0" y="32" width="70" height="15" rx="3" fill="#818CF8" stroke="#1E1B4B" strokeWidth="1.5" />
                <path d="M5 32 L5 47 M15 32 L15 47" stroke="#1E1B4B" strokeWidth="1.5" />
                
                <rect x="5" y="18" width="65" height="14" rx="3" fill="#A78BFA" stroke="#1E1B4B" strokeWidth="1.5" />
                <path d="M10 18 L10 32 M20 18 L20 32" stroke="#1E1B4B" strokeWidth="1.5" />

                <rect x="10" y="5" width="60" height="13" rx="3" fill="#C084FC" stroke="#1E1B4B" strokeWidth="1.5" />
                <path d="M15 5 L15 18 M25 5 L25 18" stroke="#1E1B4B" strokeWidth="1.5" />

                <g transform="translate(15, -15)">
                  <ellipse cx="25" cy="18" rx="12" ry="4" fill="#312E81" stroke="#1E1B4B" strokeWidth="1.5" />
                  <path d="M13 18 L13 22 C13 25, 37 25, 37 22 L37 18" fill="#312E81" stroke="#1E1B4B" strokeWidth="1.5" />
                  <polygon points="25,5 47,13 25,21 3,13" fill="#1E1B4B" />
                  <path d="M25,13 L42,16 L43,26" fill="none" stroke="#F59E0B" strokeWidth="1.5" strokeLinecap="round" />
      ...
      <div>
        <div className="flex items-center justify-between pb-1">
          <h2 className="text-[#0F172A] font-bengali font-extrabold text-xl sm:text-2xl tracking-tight">আমার অ্যাকাউন্ট</h2>
        </div>
      ...
      <div className="space-y-4">
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-[#1F2937] font-bold font-bengali text-lg leading-tight group-hover:text-[#7C3AED] transition-colors">ব্যক্তিগত তথ্য</h3>
              <p className="text-[#6B7280] font-bengali text-sm mt-1">নাম, ফোন নম্বর, ইমেইল, ব্যাচ ইত্যাদি</p>
            </div>
          </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-[#1F2937] font-bold font-bengali text-lg leading-tight group-hover:text-[#7C3AED] transition-colors">সাবস্ক্রিপশন</h3>
              <p className="text-[#6B7280] font-bengali text-sm mt-1">বর্তমান প্ল্যান, আপগ্রেড, মেয়াদ</p>
            </div>
          </div>
          <ChevronRight className="w-5 h-5 text-slate-400 group-hover:text-[#7C3AED] transition-all group-hover:translate-x-1" />
        </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-[#1F2937] font-bold font-bengali text-lg leading-tight group-hover:text-[#7C3AED] transition-colors">রুটিন তৈরি</h3>
              <p className="text-[#6B7280] font-bengali text-sm mt-1">নিজের পড়ার রুটিন তৈরি ও ম্যানেজ করুন</p>
            </div>
          </div>
          <ChevronRight className="w-5 h-5 text-slate-400 group-hover:text-[#7C3AED] transition-all group-hover:translate-x-1" />
        </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-[#1F2937] font-bold font-bengali text-lg leading-tight group-hover:text-[#7C3AED] transition-colors">নোটিফিকেশন</h3>
              <p className="text-[#6B7280] font-bengali text-sm mt-1">সকল আপডেট ও ঘোষণা</p>
            </div>
          </div>
          <ChevronRight className="w-5 h-5 text-slate-400 group-hover:text-[#7C3AED] transition-all group-hover:translate-x-1" />
        </div>
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-[#1F2937] font-bold font-bengali text-lg leading-tight group-hover:text-[#7C3AED] transition-colors">প্রোফাইল ও সেটিংস</h3>
              <p className="text-[#6B7280] font-bengali text-sm mt-1">অ্যাভাটার, পাসওয়ার্ড, অ্যাকাউন্ট সেটিংস</p>
            </div>
          </div>
          <ChevronRight className="w-5 h-5 text-slate-400 group-hover:text-[#7C3AED] transition-all group-hover:translate-x-1" />
        </div>
      ...
      <div className="bg-white rounded-[32px] shadow-[0_8px_30px_rgba(0,0,0,0.02)] border border-slate-100 p-6 relative overflow-hidden">
        <div className="flex items-center justify-between mb-5 border-b border-slate-50 pb-3">
          <div className="flex items-center gap-2.5">
            ...
            <h2 className="text-xl font-extrabold font-bengali text-[#0F172A] leading-none">আমার মক টেস্ট অগ্রগতি</h2>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          ...
          <div className="bg-[#F8FAFC] p-4 rounded-2xl flex flex-col items-center justify-center text-center">
            ...
            <span className="text-xs font-bengali text-slate-500">গড় সঠিক উত্তর</span>
        ...
        <div className="mt-6">
          ...
          <div className="h-28 w-full flex items-end justify-between gap-2.5">
            {examResults.slice(-7).map((res, idx) => {
              ...
              return (
                <div key={idx} className="flex flex-col items-center flex-1 gap-2 group relative h-full justify-end">
                  ...
                  <span className="text-[9px] text-slate-400 font-bold font-bengali">টেস্টসূচি {toBn(idx+1)}</span>
                </div>
              );
            })}
      ...
      {showPersonalInfoModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-fade-in">
          <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl border border-slate-100 max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-bold font-bengali text-indigo-700 uppercase tracking-wider mb-2">পূর্ণ নাম (Full Name)</label>
                <input 
                  type="text" 
                  value={editName}
              ...
              <div>
                <label className="block text-xs font-bold font-bengali text-indigo-700 uppercase tracking-wider mb-2">উক্তি (Favorite Quote)</label>
                <input 
                  type="text" 
                  value={editQuote}
              ...
              <div>
                <label className="block text-xs font-bold font-bengali text-indigo-700 uppercase tracking-wider mb-2">প্রতিষ্ঠান (Institution)</label>
                <input 
                  type="text" 
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold font-bengali text-indigo-700 uppercase tracking-wider mb-2">শ্রেণী/গ্রুপ (Class/Group)</label>
                  <select 
                    value={editClass}
                ...
                <div>
                  <label className="block text-xs font-bold font-bengali text-indigo-700 uppercase tracking-wider mb-2">ব্যাচ badge (Batch Group)</label>
                  <input 
                    type="text" 
                    value={editBatch}
          ...
          <div className="bg-white rounded-3xl w-full max-w-lg p-6 shadow-2xl border border-slate-100 max-h-[92vh] overflow-y-auto">
            ...
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-extrabold font-bengali text-slate-900">পড়ার রুটিন তৈরি করুন</h3>
              </div>
              <button 
            ...
            <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-3 mb-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                <div>
                  <span className="text-[10px] font-bold text-slate-400 block mb-1">দিন নির্বাচন</span>
                  <select 
                    value={selectedDay}
                ...
                <div>
                  <span className="text-[10px] font-bold text-slate-400 block mb-1">বিষয় নির্বাচন</span>
                  <select 
                ...
                <div>
                  <span className="text-[10px] font-bold text-slate-400 block mb-1">সময় নির্বাচন</span>
                  <select 
                    value={selectedTime}
              ...
              >
                ...
                {routineLoading ? "যুক্ত করা হচ্ছে..." : "রুটিন স্লট যুক্ত করুন"}
              </button>
            </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-md p-6 shadow-2xl border border-slate-100 max-h-[85vh] overflow-hidden flex flex-col">
            ...
            <div className="flex justify-between items-center mb-3">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-xl font-bold font-bengali text-slate-900">নোটিফিকেশন সেন্টার</h3>
              </div>
              <button 
              ...
              {notifications.map((noti) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed font-bengali break-words">{noti.message}</p>
                  </div>
                </div>
              ))}
            ...
            <div className="bg-white rounded-3xl w-full max-w-sm overflow-hidden shadow-2xl border border-slate-100">
               <div className="p-6">
                  <div className="flex justify-between items-center mb-5">
                     ...
                     <button onClick={() => setShowGeneralSettingsModal(false)} className="w-[34px] h-[34px] flex items-center justify-center bg-slate-100 hover:bg-slate-200 transition-colors rounded-full text-slate-500">
                        <X className="w-4 h-4" />
                     </button>
                  </div>
                  ...
                  <div className="space-y-3.5">
                     ...
                     >
                        <div className="flex items-center gap-3">
                           ...
                           <div className="text-left">
                              <div className="font-extrabold text-[#EF4444] text-sm font-bengali">অ্যাকাউন্ট লগ আউট</div>
                              <div className="text-[11px] text-red-400 font-bengali mt-0.5">নিরাপদে আপনার সেশন বন্ধ করুন</div>
                           </div>
                        </div>
            ...
            <div className="space-y-2 mb-5 max-h-[250px] overflow-y-auto pr-1">
              {['বিজ্ঞান', 'মানবিক', 'বাণিজ্য', 'এইচএসসি বিজ্ঞান', 'এইচএসসি মানবিক', 'এইচএসসি বাণিজ্য', 'এইচএসসি', 'এডমিশন'].map(grp => (
                <label key={grp} className={`flex items-center gap-3 p-2.5 rounded-xl border cursor-pointer transition-colors ${classChangeGroup === grp ? 'bg-indigo-50 border-indigo-200' : 'bg-white border-slate-100 hover:bg-slate-50'}`}>
                  ...
                  />
                  <span className={`font-bengali text-sm font-bold ${classChangeGroup === grp ? 'text-indigo-800' : 'text-slate-700'}`}>{grp}</span>
                </label>
              ))}
            </div>
