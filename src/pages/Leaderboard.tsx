  ...
  if (rank === 1) {
    return (
      <div className="relative flex justify-center items-center w-10 h-10 mx-auto">
        ...
        <span className="relative text-white font-bold text-sm z-10 -mt-3">
          {rank}
        </span>
      </div>
    );
    ...
    return (
      <div className="relative flex justify-center items-center w-10 h-10 mx-auto">
        <svg
          ...
          viewBox="0 0 32 40"
          fill="none"
          className="absolute top-0 drop-shadow-sm"
        >
...
export default function Leaderboard() {
  ...
  // Resolve the effective class to view. Admin's previewClass overrides their actual class.
  // We use userData?.class, fallback to "নবম শ্রেণী (Class 9)" instead of English "Class 9".
  const userClass = previewClass || userData?.class || "নবম শ্রেণী (Class 9)";
  ...
  useEffect(() => {
    const fetchLeaderboard = async () => {
      setLoading(true);
      try {
        // Query users by class only, avoiding composite index requirements.
        const q = query(
          collection(db, "users"),
        ...
        // Sort in memory (descending points)
        data.sort((a, b) => {
          const pb = b.points || 0;
          const pa = a.points || 0;
      ...
      <div className="bg-card rounded-[24px] shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-slate-100 overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100 bg-card">
                <th className="py-4 px-6 font-semibold text-slate-400 text-[13px] w-20 text-center uppercase tracking-wider">
                  Rank
                </th>
                <th className="py-4 px-6 font-semibold text-slate-400 text-[13px] uppercase tracking-wider">
                  Student Name
                </th>
                <th className="py-4 px-6 font-semibold text-slate-400 text-[13px] text-right uppercase tracking-wider">
                  Points
                </th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                <tr>
                  <td
                    colSpan={3}
                    className="py-16 text-center text-slate-400 font-bengali"
                  >
                ...
                users.map((u, i) => (
                  <tr
                    key={u.id}
                    className={`border-b border-slate-50 last:border-none hover:bg-muted/80 transition-colors ${userData?.uid === u.id ? "bg-[#fff8f0] hover:bg-[#fff5e6]" : i === 0 ? "bg-amber-50/30" : ""}`}
                  >
                    ...
                    <td className="py-4 px-6">
                      <div className="flex items-center gap-4">
                        <div
                          className={`w-11 h-11 rounded-full flex items-center justify-center font-bengali text-[15px] font-bold overflow-hidden shadow-sm shrink-0 border border-white ${getAvatarColor(i)}`}
                        >
                          {u.photoURL ? (
                            <img
                              src={u.photoURL}
                            ...
                            getInitials(u.fullName || "N A")
                          )}
                        </div>
                        <div className="flex items-center gap-2.5">
                          <span className="font-semibold text-foreground text-[15.5px] font-bengali">
                            {u.fullName || "No Name"}
                          </span>
                          {u.isPro && (
                            <Crown className="w-4.5 h-4.5 text-amber-500 fill-amber-500 drop-shadow-sm" />
                          )}
                          {userData?.uid === u.id && (
                            <span className="bg-slate-800 text-white text-[10px] font-bold px-2 py-0.5 rounded-full tracking-wider shadow-sm">
                              YOU
                            </span>
                          )}
                        </div>
                    ...
                    <td className="py-4 px-6 text-right">
                      <div className="flex items-center justify-end gap-2 pr-2">
                        <span
                          className={`font-bold font-mono text-[19px] ${userData?.uid === u.id ? "text-[#e65c00]" : i === 0 ? "text-[#e65c00]" : "text-slate-700"}`}
                        >
                          {u.points || 0}
                        </span>
                        <Star
                          className={`w-5 h-5 ${i < 3 || userData?.uid === u.id ? "fill-[#F59E0B] text-[#F59E0B] drop-shadow-sm" : "fill-slate-300 text-slate-300"}`}
                        />
                      </div>
                    </td>
                  </tr>
        ...
        <div className="bg-[#f8f9fe] p-5 md:p-6 border-t border-slate-100 flex items-center justify-between rounded-b-[24px]">
          <div className="flex items-center gap-3.5">
            ...
            <span className="font-bengali text-slate-700 font-medium text-[15px]">
              নিয়মিত পড়াশোনা করুন, এগিয়ে থাকুন সবার থেকে! 🎉
            </span>
          </div>
